...
function Row({
  ...
  return (
    <button
      ...
      {logged ? (
        <span className="shrink-0 inline-flex items-center gap-2 text-aa-caption text-aa-text-secondary">
          {d?.rating != null && (
        ...
        <span className="shrink-0 inline-flex items-center px-2 py-0.5 rounded-aa-pill text-aa-10 font-bold uppercase tracking-wide bg-aa-warning-soft text-aa-warning-strong">
          No debrief
        </span>
      )}

      <span className="w-24 shrink-0 text-right text-aa-caption font-semibold text-aa-primary">
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        {stats.map((s) => (
          <div key={s.label} className={`aa-card-base p-4`}>
      ...
      {needs.length === 0 && logged.length === 0 ? (
        <div className={`aa-card-base p-aa-6 py-16 text-center`}>
        ...
        <div className="space-y-6">
          {needs.length > 0 && (
            <div className={`aa-card-base overflow-hidden`}>
